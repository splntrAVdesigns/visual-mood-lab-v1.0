...
**Status:** Phases 0–4 shipped (Phase 4's LFO-based modulation is complete and live; audio input is confirmed not yet wired and is deferred by design — see callout in §7, not a blocker); Phase 4.5, 4.6, and 4.7 all complete — **Phase 4.8 (tile sound presets) Stages 1–2 complete, Field Lines and Acid Melt are shipped, working instruments**; Stage 3 recalibrated into two 4-tile batches (see §7) — batch 1 next; Phase 5 (Playground) follows
**Last updated:** 2026-08-13 (rev 9 — §7 Phase 4.8 Stages 1–2 marked complete; Stage 3 restructured from a flat 6-tile list into two 4-tile batches sequenced by engine reuse rather than tile order, per what Stage 2 actually revealed about implementation risk; full detail and rationale in the companion `SOUND_PRESETS_CONCEPT.md`, also updated this pass)
...
### Phase 4 — Modulation bus ✅ LFO-BASED MODULATION COMPLETE — audio input deferred
...
**Exit criterion met for LFO-based modulation; audio input is not.**

> **Resolved during the Phase 5 readiness review (rev 7):** confirmed — the Settings dialog's "Audio reactivity" toggle isn't stale UI hiding a working feature, it's accurately describing an unfinished one. `navigation/`'s only audio-related code is `audioEnabled`/`setAudioEnabled`, inert boolean state on `usePlaybackStore` feeding a hard-coded-`disabled` toggle; nothing there consumes it. `lib/modulation/` itself wasn't available to inspect directly, so this is confirmed from the consumer side, not the source — worth a direct look at `lib/modulation/` specifically whenever full audio integration is actually picked up, but not before, per the decision below.
>
> **Deferred by design, not by oversight:** this lines up with the stated roadmap sequencing — Playground and full audio integration sit together near the end, just ahead of Export, Blend layers, MIDI, and Timeline. LFO-driven modulation (clock, right-click assign, visual indicator) is real and shipped; it's a complete, working system on its own and doesn't need audio to be useful. Playground's `@mod`-tagged controls will modulate correctly against LFO sources today — audio as a *source option* simply isn't one of them yet. Nothing here blocks Phase 5.
...
### Phase 4.8 — Tile sound presets ("mood tiles as instruments") ← Stages 1–2 ✅ COMPLETE, Stage 3 next

A mood tile's own live parameter values also drive a small audio subgraph — motion becomes sound organically, using data the tile is already producing. Architecturally a sibling to Phase 4's modulation bus, not a dependency of it: that system is audio-**in** (sound → visuals); this is audio-**out** (visuals → sound). Full concept doc, updated this pass: `SOUND_PRESETS_CONCEPT.md`.
...
- [x] **Stage 2 — Proof of concept, Field Lines + Acid Melt:** shipped well past the original 7-control scope. See `SOUND_PRESETS_CONCEPT.md` §"What actually shipped" for the full record; headline items:
  ...
  - **Graze-to-pluck** — a new `triggerMode: 'event'` model where a sketch fires discrete trigger events itself (`p.pluck(x)` → sandbox → `pluckTileAudio()`) instead of a hover-gated schedule. This is the single biggest departure from the original concept doc, and the reason Stage 3's tile list below is organized around it
  - **Note rack** replaced the single-value Key select: up to 3 simultaneous roots, a real chord on the pad engine, a merged/widened scale pool on the arp engine, and — as of this pass — deselectable to zero as a per-tile mute independent of the master Sound toggle
  - **Humanize & Swing**, added beyond the original plan's scope entirely, each with a genuinely different implementation per engine rather than one shared mechanism (arp: timing/pitch/velocity jitter, or accent-based swing in event mode; pad: slow detune drift, or alternating tremolo-depth swing)
  - Envelope question (§"Open decisions" below) resolved as **option 1 (fixed per-engine) plus per-preset bindings** — never revisited toward 2/3/4, and no longer expected to be
- [ ] **Stage 3 — Remaining synthesized tiles, split into two batches by implementation risk, not tile order** (see `SOUND_PRESETS_CONCEPT.md` for full rationale):
  ...
  - **Batch 2:** Wound Thread (needs a genuinely new `abstract` engine), SVG Particle (reuses `ArpEngine` but needs design work on the emission→pluck mapping), Digital Matrix + Grid Snake (carried from what was Stage 4 — still blocked on owner-supplied `.wav` files)
- [ ] **Stage 4 folded into Stage 3, batch 2** — Digital Matrix and Grid Snake's sample-loader work happens alongside batch 2 rather than as a separate later stage, since nothing about them requires batch 1 to finish first except sequencing preference; still blocked on the owner supplying `.wav` files, unchanged from the original plan
...
**Exit criterion:** all 10 tiles are genuinely playable instruments, each stage's checkpoint held before moving to the next, and the sound-binding schema — now including `triggerMode`, `defaultOctave`/`defaultNotes`, and the humanize/swing fields, none of which existed when this criterion was first written — is stable enough that Playground doesn't need to redesign it later to let people attach presets to their own new assets.
...
7. **Media playback controls for uploaded image/video assets** — LUTs, auto hue-cycling, playback speed. A natural extension of the `BASE_CONTROLS`-plus-type-specific-extras concept already named in §5, scoped entirely to the Phase 1 upload pipeline rather than anything Playground touches — doesn't block Phase 5. Considered and deliberately deferred during the Phase 5 readiness review (rev 7): pairs more naturally with Phase 6's export/color work than with anything Playground needs, since Playground's own loop is about authoring new shader/p5 code, not filtering existing media.
