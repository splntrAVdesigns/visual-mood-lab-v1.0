...
- Phase 4.9 core (mic + track modulation input) — **now including the mobile lifecycle stabilization work from this revision**
- Phase 4.96 (VFX rack expansion) — **new, complete this revision**
- Modulation system — six-fix pass + `baseParams` systemic fix (this revision)
- Mood tile library — 90 assets
...
- **Phase 4.98** (Floating Sidecar Panels, desktop) — **shipped (rev 17): merged to `main` as `a3cfbe9` on 2026-09-21; owner-verified on the Vercel preview that the app loads and functions in Safari and Firefox.** Detach Sound / VFX / VCapture / Modulation from the fixed left stack into free-floating panels, plus an "expand one, collapse the others" accordion in stack mode. Design: `VISUAL_MOOD_LAB_FLOATING_PANELS_INTEGRATION_PLAN.md` §3; results, deviations, and open items: `SPRINT_FLOATING_PANELS.md` §11
...
### Phase 4.95 — Media & Graphic Asset Library (est. 4–6 days) — planned, not started
...
**Status check (rev 11):** not started. No change since rev 10. Still recommended to run ahead of Playground and Phase 5.5.
...
| 6 | SVG mask rendering | Offscreen GL texture (rasterize SVG → texture, composite in the existing WebGL pass) **vs** DOM-level `mask-image`/`clip-path` (native SVG alpha, separate code path from the GL compositor) | Phase 5.5 |
