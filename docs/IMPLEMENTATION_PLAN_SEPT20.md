...
### Phase 4.99.2 A–B — Performance baseline and Particle Cube clarity

- **A — Instrumentation:** an opt-in `?perf=1` bounded trace now records host frame gaps and anonymous per-tile render/VFX CPU durations, pass count, pixel count and sandbox FPS. The device baseline is an open acceptance gate; record iPhone Safari and desktop results before tuning adaptive thresholds. See `docs/PHASE_4_99_2_AB_PERFORMANCE_AND_PARTICLE_CUBE.md` for the scenario matrix.
- **B — Correctness and visibility:** VFX passes now agree on one aspect-preserving stage-bounded viewport without reallocating the presentation canvas; Particle Cube reuses the established 28-color-bucket p5 points technique, with sparse halos, camera-facing luminous cores, view-aware color depth and cached jitter.
- **Next gate:** compare visual rotations and 1600x800 VFX GPU/relay pixels in a browser, then measure before/after p50/p95 frame pacing and resources on a real iPhone. Adaptive quality, worker migration and additional frame suppression remain outside A–B until the baseline exists.
